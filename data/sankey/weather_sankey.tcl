...
foreach city $cities {
  ...
  for {set month 1} {$month <= 12} {incr month} {
    set month_model($city,$month) [create_charts_data_model -rows 31 -columns 5]

    set_charts_data -model $month_model($city,$month) -header -column 0 -name value -value "Conditions"
    set_charts_data -model $month_model($city,$month) -header -column 1 -name value -value "Day"
    set_charts_data -model $month_model($city,$month) -header -column 2 -name value -value "Value"
    set_charts_data -model $month_model($city,$month) -header -column 3 -name value -value "PathId"
    set_charts_data -model $month_model($city,$month) -header -column 4 -name value -value "Attributes"

    # set named colors and named images for conditions
    set_charts_data -model $month_model($city,$month) -column 0 -name column_type \
...
# create sankey plot
set plot [create_charts_plot -type sankey -model $month_model($current_city,1) \
  -columns {{from 1} {to 0} {value 2} {pathId 3} {attributes 4}} \
  -title "Weather for $current_city_title in $current_month"]

set_charts_property -plot $plot -name options.orientation -value Vertical
set_charts_property -plot $plot -name options.valueLabel  -value 1
set_charts_property -plot $plot -name text.internal       -value 0
...
#set_charts_property -plot $plot -name margins.outer.left   -value 5%
#set_charts_property -plot $plot -name margins.outer.right  -value 5%
#set_charts_property -plot $plot -name margins.outer.top    -value 5%
#set_charts_property -plot $plot -name margins.outer.bottom -value 5%
