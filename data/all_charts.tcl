...
set plot1 [create_charts_plot -model $model1 -type adjacency -columns "name=1,connections=3,name=0,groupId=2" -title "adjacency"]
...
set plot4 [create_charts_plot -model $model4 -type bubble -columns "group=0,value=1" -title "Bubble Group"]
...
set plot6 [create_charts_plot -model $model6 -type delaunay -columns "x=6,y=5,name=1" -title delaunay]

# Dendrogram

set model7 [load_charts_model -csv data/flare.csv -comment_header]

set plot7 [create_charts_plot -model $model7 -type dendrogram -columns "name=0,value=1" -title "dendrogram"]
...
set plot9 [create_charts_plot -model $model9 -type forcedirected -columns "node=1,connections=3,name=0,groupId=2" -title "Force Directed"]

# Geometry Plot

set model10 [load_charts_model -tsv data/states.tsv -comment_header]

set plot10 [create_charts_plot -model $model10 -type geometry -columns "name=0,geometry=1" -title "geometry"]
...
set plot12 [create_charts_plot -model $model12 -type parallel -columns "x=0,y=1 2 3 4 5 6 7" -title "parallel"]

# Pie Plot

set model13 [load_charts_model -csv data/ages.csv -first_line_header -column_type "1#integer"]

set plot13 [create_charts_plot -model $model13 -type pie -columns "label=0,value=1" -title "pie chart"]

# Radar Plot

set model14 [load_charts_model -csv data/radar.csv -first_line_header]

set plot14 [create_charts_plot -model $model14 -type radar -columns "name=0,value=1 2 3 4 5" -title "Radar Plot"]
...
set plot17 [create_charts_plot -model $model17 -type hierscatter -columns "x=6,y=5,name=1,group=4 3 2" -title "airports"]

# Sunburst Plot

set model18 [load_charts_model -json data/flare.json]

set plot18 [create_charts_plot -model $model18 -type sunburst -columns "names=0,value=1" -title "sunburst"]

# TreeMap Plot

set model19 [load_charts_model -json data/flare.json]

set plot19 [create_charts_plot -model $model19 -type treemap -columns "names=0,value=1" -title "tree map"]

# XY Plot

set model20 [load_charts_model -tsv data/multi_series.tsv -comment_header -column_type "time:format=%Y%m%d,oformat=%F"]
