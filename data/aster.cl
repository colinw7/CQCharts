...
for (i = 0; i < nr; ++i)
  get_charts_data -model $modelId -row $i -column 2 -name value
  score = _rc

  get_charts_data -model $modelId -row $i -column 3 -name value
  weight = _rc
...
set_model -model $modelId -column_type "4#color"
...
create_text_shape -plot $plotId -x 0 -y 0 -text $average -align center
textId = _rc
