...
exprSlot()
{
  ...
  int column1 = test_->cmds()->processExpression(model, function, column, expr);
...
foldSlot()
{
  ...
  test_->cmds()->foldModel(modelData, text);
...
typeSetSlot()
{
  ...
  if (! ok) {
    test_->errorMsg("Invalid column number '" + numStr + "'");
    return;
  }
