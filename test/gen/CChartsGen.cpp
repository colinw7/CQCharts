  ...
  CArgv::visit(argc, argv,
   ...
   [&]() { // help
     std::cerr <<
      ...
      " -s <seed>        ; random seed\n"
      " -from_to         ; edge (from/to)\n"
      " -series <series> ; time series (X and N-1 Ys)\n"
      ...
      " -size            ; name size\n"
      " -header          ; add header\n"
      " -group           ; add group (for -fromTo)\n"
      " -labels          ; add labels (for scatter)\n"
      " -discreet        ; discreet (duplicate) labels (for -scatter -labels)\n"
      " -stacked         ; stacked values (for -series)\n"
      " -hier            ; hierarchical data\n"
      " -depth <depth>   ; hierarchical depth\n"
      " -values          ; single column values"
      "\n";
      exit(0);
   });
