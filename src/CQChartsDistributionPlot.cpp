...
addProperties()
{
  ...
  addPropI("options", "extraValues", "", "Extra values");
...
void
CQChartsDistributionPlot::
setExtraValues(const CQChartsValueList &extraValues)
{
...
calcRange() const
{
  ...
  // set main bucketer from first group bucketer and values
  int groupInd = 0;
  ...
  for (auto &groupValues : groupData_.groupValues) {
    ...
    if      (isDensity()) {
      ...
      for (auto &bucketValues : values->bucketValues) {
        ...
        scatterObj->connectDataChanged(this, SLOT(updateSlot()));
  ...
  if (isVertical()) {
    ...
    if (! found) {
      ...
      for (const auto &plotObj : plotObjs_) {
        ...
        if (barObj && barObj->bucketValueX(w.x, x, align)) {
          found = true;
          w1.x = x;
          break;
        }
    ...
    if (! found) {
      ...
      for (const auto &plotObj : plotObjs_) {
        ...
        if (barObj && barObj->bucketValueY(w.y, y, align)) {
          found = true;
          w1.y = y;
          break;
        }
...
bool
CQChartsDistributionBarObj::
bucketValueX(double value, double &x, Qt::Alignment align) const
{
  double value1, value2;
...
bool
CQChartsDistributionBarObj::
bucketValueY(double value, double &y, Qt::Alignment align) const
{
  double value1, value2;
...
{
  if (plot_ && distributionPlot_)
    disconnect(distributionPlot_, SIGNAL(customDataChanged()), this, SLOT(updateWidgets()));

  distributionPlot_ = dynamic_cast<CQChartsDistributionPlot *>(plot);

  CQChartsGroupPlotCustomControls::setPlot(plot);

  if (distributionPlot_)
    connect(distributionPlot_, SIGNAL(customDataChanged()), this, SLOT(updateWidgets()));
}
